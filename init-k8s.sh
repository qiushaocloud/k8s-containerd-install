...
if [ "$controlPlaneEndpoint" != "" ]; then
    ...
    if [ "$k8sToken" != "" ]; then
        ...
        #kubeadm init --kubernetes-version v1.26.5 --image-repository registry.aliyuncs.com/google_containers --apiserver-advertise-address=$apiserverAdvertiseAddress --apiserver-bind-port=$apiserverBindPort --pod-network-cidr=10.244.0.0/16 --service-cidr=10.1.0.0/16 --upload-certs --control-plane-endpoint=$controlPlaneEndpoint --token=$k8sToken --token-ttl=0 --cri-socket unix:///run/containerd/containerd.sock
    else
    ...
    if [ "$k8sToken" != "" ]; then
        sed -i "s/<K8S_TOKEN>/$k8sToken/g" kubeadm-config.yml
        sed -i "s/ttl: 24h0m0s/ttl: '0'/g" kubeadm-config.yml
        #kubeadm init --kubernetes-version v1.26.5 --image-repository registry.aliyuncs.com/google_containers --pod-network-cidr=10.244.0.0/16 --service-cidr=10.1.0.0/16 --apiserver-advertise-address=$apiserverAdvertiseAddress --apiserver-bind-port=$apiserverBindPort --upload-certs --token=$k8sToken --token-ttl=0 --cri-socket unix:///run/containerd/containerd.sock
    else
        sed -i "/指定token/d" kubeadm-config.yml
        sed -i "/<K8S_TOKEN>/d" kubeadm-config.yml
        sed -i "/ttl: 24h0m0s/d" kubeadm-config.yml
